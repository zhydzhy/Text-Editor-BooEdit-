...
public:
    ...
    // Make the cursor column plus one to make it visually 'right'
    void moveRight(); 

    // Make the cursor column plus one to make it visually 'left'
    void moveLeft();

    // Make the cursor column to one to make it visually 'left most'
    void moveHome();

    // Make the cursor column to the last character of the line plus one
    void moveEnd();

    // Make the cursor line, column back to original (helper for undo())
    void moveBack(int line, int col);

    // Inserts a character at the cursor position, add one to cursor column
    void insertChar(char c);

    // Deletes a character at the cursor position, minus one to cursor column
    void deleteChar();

    // Returns the character deleted from deleteChar()
    char getBackUpChar();

    // Inserts a new line to the text
    void newLine();

    // Deletes a line to the text, along with the characters 
    void deleteLine();

    // Return the line(characters) deleted from deleteLine()
    std:: string getBackUpStr();

    // Inserts the given text to where the cursor is located
    void insertLine(std::string& text);

    // Moves the cursor up a line, ending at the right most character
    void moveUp();

    // Moves the cursor down a line, ending at the right most character
    void moveDown();
